...
def create_single_pixel_channel(num_leds: int, position: int, brightness: int) -> list[int]:
...
async def blink_one_led_continuously(channel: Channel, num_leds: int, i: int, brightness_queue):
    while True:
        current_brightness, _ =  brightness_queue.get()
        print("Blinking LED"+str(i)+" at brightness "+str(current_brightness))
        await channel.set_values(create_single_pixel_channel(num_leds, i, current_brightness))
        await asyncio.sleep(1)
        all_off_array = [0]*get_channel_width(num_leds) # Brightness is 0 and all LEDs are black
        await channel.set_values(all_off_array)
        await asyncio.sleep(1)

async def calibration_blink(ip_address:str, num_leds:int, brightness_queue) -> asyncio.Task[NoReturn]:
    print("Setting up LEDs")
    channel = await setup_artnet_leds(ip_address, num_leds)
    print("LEDs are ready")
    return asyncio.ensure_future(blink_one_led_continuously(channel, num_leds, 1, brightness_queue))
